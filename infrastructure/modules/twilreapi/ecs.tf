...
resource "aws_ecs_task_definition" "old_appserver" {
  family                   = "${var.old_service_name}-appserver"
  network_mode             = "awsvpc"
  requires_compatibilities = ["FARGATE"]
  container_definitions = data.template_file.old_appserver_container_definitions.rendered
  task_role_arn = aws_iam_role.ecs_task_role.arn
  execution_role_arn = aws_iam_role.task_execution_role.arn
  cpu = 512
  memory = 1024
...
resource "aws_ecs_service" "old_appserver" {
  name            = aws_ecs_task_definition.old_appserver.family
  cluster         = aws_ecs_cluster.cluster.id
  task_definition = aws_ecs_task_definition.old_appserver.arn
  desired_count   = var.appserver_min_tasks
  launch_type = "FARGATE"
...
resource "aws_ecs_task_definition" "old_worker" {
  family                   = "${var.old_service_name}-worker"
  network_mode             = "awsvpc"
  requires_compatibilities = ["FARGATE"]
  container_definitions = data.template_file.old_worker_container_definitions.rendered
  task_role_arn = aws_iam_role.ecs_task_role.arn
  execution_role_arn = aws_iam_role.task_execution_role.arn
  cpu = 512
  memory = 1024
...
resource "aws_ecs_service" "old_worker" {
  name            = "${var.old_service_name}-worker"
  cluster         = aws_ecs_cluster.cluster.id
  task_definition = aws_ecs_task_definition.old_worker.arn
  desired_count   = var.worker_min_tasks
  launch_type = "FARGATE"
...
data "template_file" "old_appserver_container_definitions" {
  template = file("${path.module}/templates/appserver_container_definitions.json.tpl")
...
data "template_file" "old_worker_container_definitions" {
  template = file("${path.module}/templates/worker_container_definitions.json.tpl")
